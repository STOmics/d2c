...
source(normalizePath(gsub("11b_knee_execute.R", "00_knee_CL.R", gsub(needle, "", args[match]))))

# Import data supplied by the user
vec_values <- as.numeric(fread(vals_file)[[as.character(column_name)]])

# Call knee
estimated_knee_threshold <- get_density_threshold_CL(vec_values, "jaccard", as.logical(as.numeric(as.character(logTransform))))
